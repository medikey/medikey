...
export interface NPoolOpts<T extends NRelay> {
  ...
  /** Determines the relays to use for making `REQ`s to the given filters. To support the Outbox model, it should analyze the `authors` field of the filters. */ reqRouter(filters: NostrFilter[]): ReadonlyMap<string, NostrFilter[]> | Promise<ReadonlyMap<string, NostrFilter[]>>;
  /** Determines the relays to use for publishing the given event. To support the Outbox model, it should analyze the `pubkey` field of the event. */ eventRouter(event: NostrEvent): string[] | Promise<string[]>;
}
/**
 ...
 */ export declare class NPool<T extends NRelay = NRelay> implements NRelay {
  ...
  /** Returns a new pool instance that uses the given relays. Connections are shared with the original pool. */ public group(urls: string[]): NPool<T>;
