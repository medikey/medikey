/** @jsxRuntime automatic *//** @jsxImportSource npm:react@^18.0.0 *//** @jsxImportSourceTypes npm:@types/react@^18.0.0 *//** @jsxFactory React.createElement *//** @jsxFragmentFactory React.Fragment */import { Suspense } from 'npm:react@^18.0.0';

import { useAuthor } from './useAuthor.ts';
import { useCurrentUser } from './useCurrentUser.ts';
import { useLoginActions } from './useLoginActions.ts';
import { useSocialFeed } from './useSocialFeed.ts';

import type { NostrEvent } from 'jsr:@nostrify/nostrify@^0.46.4';

function App() {
  const { user, metadata } = useCurrentUser();

  const login = useLoginActions();

  function renderLogin() {
    if (user) {
      if (metadata.name) {
        return <div>Welcome back, {metadata.name}!</div>;
      }

      return <div>You: {user.pubkey}</div>;
    }

    return (
      <button
        type='button'
        onClick={() => login.extension()}
      >
        Log in
      </button>
    );
  }

  return (
    <div>
      <h2>Login</h2>
      {renderLogin()}

      <h2>Notes</h2>
      <Suspense fallback={<div>Loading...</div>}>
        <Feed />
      </Suspense>
    </div>
  );
}

function Feed() {
  const feed = useSocialFeed();

  return (
    <>
      {feed.data?.map((note) => <FeedPost key={note.id} event={note} />)}
    </>
  );
}

function FeedPost({ event }: { event: NostrEvent }) {
  const author = useAuthor(event.pubkey);

  return (
    <div key={event.id} style={{ border: '1px solid gray', padding: '10px', margin: '20px 0' }}>
      <div>{author.name ?? event.pubkey}</div>
      <div>{event.content}</div>
    </div>
  );
}

export default App;
