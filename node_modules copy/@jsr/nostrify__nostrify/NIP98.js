...
/** [NIP-98](https://github.com/nostr-protocol/nips/blob/master/98.md) HTTP auth. */ export class NIP98 {
  /** Generate an auth event template from a Request. */ static async template(request, opts) {
  ...
  /** Compare the auth event with the request, throwing a human-readable error if validation fails. */ static async verify(request, opts) {
    const { maxAge = 60_000, validatePayload = [
      'POST',
      'PUT',
      'PATCH'
    ].includes(request.method), verifyEvent = _verifyEvent } = opts ?? {};
