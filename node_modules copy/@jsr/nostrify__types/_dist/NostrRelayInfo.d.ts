/** NIP-11 Relay Information Document: https://github.com/nostr-protocol/nips/blob/master/11.md */ export interface NostrRelayInfo {
  /**
   ...
   */ limitation?: {
    ...
    /** Maximum number of filter values in each subscription. Must be one or higher. */ max_filters?: number;
    /**
     ...
     */ max_limit?: number;
    /** Maximum length of subscription id as a string. */ max_subid_length?: number;
    /** In any event, this is the maximum number of elements in the `tags` list. */ max_event_tags?: number;
    /**
    ...
    /** New events will require at least this difficulty of PoW, based on [NIP-13](https://github.com/nostr-protocol/nips/blob/master/13.md), or they will be rejected by this server. */ min_pow_difficulty?: number;
    /**
    ...
    /** This relay requires payment before a new connection may perform any action. */ payment_required?: boolean;
    /**
    ...
    /** `created_at` lower limit. */ created_at_lower_limit?: number;
    /** `created_at` upper limit. */ created_at_upper_limit?: number;
    [key: string]: unknown;
  };
  /** Event Retention. */ retention?: Array<{
    time: number | null;
    count?: number;
    kinds?: number[];
  }>;
  /** A list of two-level ISO country codes (ISO 3166-1 alpha-2) whose laws and policies may affect this relay. `EU` may be used for European Union countries. */ relay_countries?: string[];
  /** An ordered list of [IETF language tags](https://en.wikipedia.org/wiki/IETF_language_tag) indicating the major languages spoken on the relay. */ language_tags?: string[];
  /**
  ...
  /** A URL pointing to an image to be used as an icon for the relay. Recommended to be squared in shape. */ icon?: string;
  [key: string]: unknown;
}
//# sourceMappingURL=NostrRelayInfo.d.ts.map